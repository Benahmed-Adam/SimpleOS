...
struct __attribute__((packed)) multiboot_mmap_entry {
    uint32_t size;    // Taille de cette entrée (souvent 20 ou 24 octets)
    uint64_t addr;    // Adresse de début de la zone de RAM
    uint64_t len;     // Longueur de la zone de RAM
    uint32_t type;    // 1 = Disponible, tout le reste = Touche pas !
};